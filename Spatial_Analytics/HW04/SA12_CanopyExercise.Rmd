...
neighborhoods_sp <- st_as_sf(neighborhoods_crs, neighborhoods_counts)
...
# Create a histogram of tree density (tree_density) For this we have to use the neighborhoods spatial object we created before as this contains the value of tree_density
ggplot(neighborhoods_sp, aes(x = tree_density)) + 
...
#With this we see that the correlation between density and canopy is -0.3564911
```
...
# The map of the manhattan aerial photo as the first shape with the raster function and then the neighborhoods as the second making the borders on the map
map1 <-  tm_shape(manhattan) + 
  tm_raster() + 
  tm_shape(neighborhoods) +
  tm_borders(col = "black", lwd = 0.5, alpha = 0.5)

map1 #Here we can run it to see if it works. 

#map2  here we set bbox as Manhattan and include both the tree_density and avg_canopy as the polygons making two maps in one basically. 
map2 <- tm_shape(neighborhoods, bbox = manhattan) + 
          tm_polygons(c("tree_density","avg_canopy"), palette = "Greens",  
          style = "quantile", n = 7)
