...
```
By running class we can see that it is a sgbp list object that is being produced with st_intersects()

2. Why do we need to use `neighborhoods[cont,]` to view the fully contained neighborhood in the buffer plot?
I couldn't get the cont to work properly I think, at least I don't know why we use index 1 in both int and cont. But I think the plan for the code is that cont takes the containing buffer in the empire state coordinates and marks the zone closes to these coordinates? 
...
st_crs(empire_state) == st_crs(parks) #It doesn't
